    ...
    public class hyWorkGroup
    {
            ...
            foreach (hyWorkFlow wf in workFlowList)
            {
                foreach (hyStationPara stationPara in wf.process.stationParaList)
                {
                    if (stationPara.enabled)
                    {
                        ...
                        if (stationPara.station_id == 12)
                        {
                            endtimelist = stationPara.endingTime + 10;
                            
                            changeTimeList.Add(endtimelist);
        ...
        public void NewWorkFlow(int process_id, int carrier_name, int person_id, int loading_station_id)
        {
                ...
                if (process.process_id == process_id)
                {
                    ...
                    foreach (hyStationPara stationPara in process.stationParaList)
                    {
                        ...
                        if (currWorkFlow.process.stationParaList[ii].workingTime > 0)
                        {
                            ...
                            {
                                case 0:
                                    modbus_TCP.send7(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    if (currWorkFlow.process.stationParaList[ii].workingTemp > 30)
                                    {
                                        modbus_TCP.OpenTemp(ii);
                                    }

                                    break;

                                case 2:
                                    modbus_TCP.send8(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    if (currWorkFlow.process.stationParaList[ii].workingTemp > 30)
                                    {
                                        modbus_TCP.OpenTemp(ii);
                                    }
                                ...
                                case 3:
                                    modbus_TCP.send1(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    modbus_TCP.OpenTemp(ii);
                                    break;

                                case 4:
                                    modbus_TCP.send2(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    modbus_TCP.OpenTemp(ii);
                                    break;

                                case 5:
                                    modbus_TCP.send3(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    modbus_TCP.OpenTemp(ii);
                                    break;

                                case 6:
                                    modbus_TCP.send4(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    modbus_TCP.OpenTemp(ii);
                                    break;

                                case 10:
                                    modbus_TCP.send9(currWorkFlow.process.stationParaList[ii].workingTemp);
                                    if (currWorkFlow.process.stationParaList[ii].workingTemp > 30)
                                    {
                                        modbus_TCP.OpenTemp(ii);
                                    }
                                ...
                                case 11:
                                    if (currWorkFlow.process.stationParaList[ii].workingTemp > 0)
                                    {
                                        modbus_TCP.send5(currWorkFlow.process.stationParaList[ii].workingTemp);
                                        modbus_TCP.OpenTemp(ii); modbus_TCP.OpenTemp(ii);
                                    }
                                ...
                                case 12:
                                    if (currWorkFlow.process.stationParaList[ii].workingTemp > 0)
                                    {
                                        modbus_TCP.send6(currWorkFlow.process.stationParaList[ii].workingTemp);
                                        modbus_TCP.OpenTemp(ii);
                                    }
            ...
            if (lastWorkFlow != null)
            {
                ...
                currWorkFlow.max_workflow_endingtime += 5;
            }
            ...
            {// workflow
                ...
                {//staion
                    ...
                    if (workFlow.process.stationParaList[index].workingTemp > 0)
                    {
                        ...
                        {
                            case 0:
                                modbus_TCP.send7(workFlow.process.stationParaList[index].workingTemp);
                                if (workFlow.process.stationParaList[index].workingTemp > 30)
                                {
                                    modbus_TCP.OpenTemp(index);
                                }

                                break;

                            case 2:
                                modbus_TCP.send8(workFlow.process.stationParaList[index].workingTemp);
                                if (workFlow.process.stationParaList[index].workingTemp > 30)
                                {
                                    modbus_TCP.OpenTemp(index);
                                }
                            ...
                            case 3:
                                modbus_TCP.send1(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;

                            case 4:
                                modbus_TCP.send2(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;

                            case 5:
                                modbus_TCP.send3(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;

                            case 6:
                                modbus_TCP.send4(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;

                            case 10:
                                modbus_TCP.send9(workFlow.process.stationParaList[index].workingTemp);
                                if (workFlow.process.stationParaList[index].workingTemp > 30)
                                {
                                    modbus_TCP.OpenTemp(index);
                                }
                            ...
                            case 11:
                                modbus_TCP.send5(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;

                            case 12:
                                modbus_TCP.send6(workFlow.process.stationParaList[index].workingTemp);
                                modbus_TCP.OpenTemp(index);
                                break;
