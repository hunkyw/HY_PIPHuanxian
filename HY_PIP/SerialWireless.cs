...
namespace HY_PIP
{
    public class SerialWireless : SerialPort
    {
        ...
        public const int pulse_per_mm = 40;
            ...
            switch (recvStatus)
            {
                ...
                case RECV_STATUS.STATE_GOT_END1:
                    if (inData == 0x5A)
                    {
                        ...
                        //
                        GenericOp.xPos = BitConverter.ToInt16(recvBuff, 4);// 转化为 mm 单位
                        GenericOp.zPos = BitConverter.ToInt16(recvBuff, 6) / 1;// 转化为 mm 单位
                        GenericOp.xPos_servo = -BitConverter.ToInt16(recvBuff, 8);// 转化为 mm 单位
                        GenericOp.zPos_servo = -BitConverter.ToInt16(recvBuff, 10) / 1;// 转化为 mm 单位
